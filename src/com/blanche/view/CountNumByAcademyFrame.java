...
public class CountNumByAcademyFrame extends JInternalFrame {
...
//	private StudentInfo studentInfo;
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					CountNumByAcademyFrame frame = new CountNumByAcademyFrame();
	...
	public void fillTable(List<StudentInfo> students) {
		DefaultTableModel dtm = (DefaultTableModel)numOfPeopleTable.getModel();  //获取表的model，以便填充数据
		dtm.setRowCount(0);  
		...
		Set<String> mySet = new HashSet();  //创建set集合,用来填充学院,筛选掉相同的学院
		for(StudentInfo studentInfo : students) {
				mySet.add(studentInfo.getAcademy());   //将不同的学院加入到myset集合中
		}
		...
		for(int i=0;i<mySet.size();i++) {
			...
			String str = it.next();  //将set集合中的元素取出来
			currentRow.addElement(str);
			currentRow.addElement(studentInfoDao.queryByAcademy(str).size() + "");  //调用queryByAcademy方法,返回list集合,再调用size()方法统计该学院的人数
